...
| Commit | Description |
|---|---|
| `local` (rama `soroban`) | feat(mosaico): **`/subir-lote` con permiso propio + nuevo item Comercial "Consulta cursos"**. (1) **Subir Lote** dejó de ser **SUPER_ADMIN duro** (`superAdminOnly`) → permiso asignable **`COMERCIAL.SUBIR_LOTE`** (enum + catálogo `/admin/permissions` sección "Subir Lote"). Se cerró la brecha: la página [`/subir-lote`](src/app/subir-lote/page.tsx) ahora tiene `PermissionGuard` propio y el **middleware** ([`ROUTE_PERMISSIONS`](src/lib/middleware-permissions.ts)) gatea la URL directa. SUPER_ADMIN/ADMIN bypassean (ven todo). (2) **Nuevo item Comercial "Consulta cursos"** ([`/dashboard/comercial/consulta-cursos`](src/app/dashboard/comercial/consulta-cursos/page.tsx), newTab): vista **read-only** de los cursos de campaña (`/api/postgres/cursos-campaign`) con campaña/tipo/salón/**guía**/horario/fechas/**cupos** (óvalo verde/amarillo), filtros por nombre y tipo de curso. Gateado por **`COMERCIAL.CONSULTA_CURSOS`** (enum + catálogo + sidebar + middleware). Para que un rol vea/use estas opciones, el admin marca el permiso en `/admin/permissions`. |
