...
export default function StudentMovimientoAcademico({
  ...
  const ejecutar = async () => {
    ...
    try {
      ...
      const d = await r.json()
      if (!r.ok || !d.success) throw new Error(d?.error || 'No se pudo aplicar el movimiento')
      toast.success('Movimiento académico aplicado')
      onSuccess()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold text-gray-900">Movimiento Académico</h3>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2">
          ...
          {preview && preview.encontrado && !preview.bloqueadoWelcome && preview.direccion !== 'igual' && (
            <button type="button" onClick={ejecutar} disabled={!confirm || submitting}
              ...
              {submitting ? 'Aplicando…' : 'Confirmar movimiento'}
            </button>
          )}
        </div>
      </div>
