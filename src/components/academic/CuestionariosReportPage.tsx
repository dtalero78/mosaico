...
/**
 * Pantalla de resultados de cuestionarios por curso. La usan DOS páginas del
 * menú Académico › Guías con la MISMA operatividad, cambiando sólo la categoría
 * de módulo que consultan (parámetro `tipo` del endpoint):
 *
 *   /dashboard/academic/evaluaciones   → tipo="evaluacion"    (módulos "Evaluación NN")
 *   /dashboard/academic/entrenamientos → tipo="entrenamiento" (módulos "Entrenamiento NN")
 *
 * Cada una tiene su propio permiso, así que a un rol se le puede dar una sin la
 * otra; los endpoints aceptan cualquiera de los dos (requireAnyPermission).
 */
export interface CuestionariosReportPageProps {
  tipo: 'evaluacion' | 'entrenamiento'
  /** Título de la página y de la columna de módulo. */
  titulo: string
  /** Singular en minúscula para los textos ("evaluación" / "entrenamiento"). */
  sustantivo: string
  permiso: Permission
}
...
export default function CuestionariosReportPage({ tipo, titulo, sustantivo, permiso }: CuestionariosReportPageProps) {
  ...
  const fetchData = useCallback(async (f: { curso: string; salon: string }) => {
    ...
    } catch (e: any) { toast.error(e?.message || `Error al cargar ${titulo.toLowerCase()}`) } finally { setLoading(false) }
  }, [tipo, titulo])
  ...
  useEffect(() => {
    ...
    const qs = new URLSearchParams({ curso: applied.curso, tipo })
  ...
  return (
    <DashboardLayout>
      <PermissionGuard permission={permiso} showDefaultMessage>
              ...
              <table className="w-full min-w-[760px] border-collapse">
                <thead>
                  <tr className="text-left">
                    ...
                    <th className="text-xs font-semibold text-gray-600 uppercase px-3 py-3 border-b-2 border-gray-200">{sustantivo}</th>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={6} className="text-center text-sm text-gray-400 py-10">Sin resultados de {titulo.toLowerCase()} para este curso todavía.</td></tr>
